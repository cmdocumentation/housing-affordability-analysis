...
**Why this matters:** Extreme housing precarity is rare (5–6% of immigrants at any age) and doesn't get worse as people age, which is somewhat reassuring. But immigrants are still about twice as likely as non-immigrants to spend half their income on housing. While the absolute numbers are small, this gap signals real vulnerability for a meaningful group. Policy should tackle both sides: the fact that it's not widespread (limiting urgency for some interventions) and the fact that it consistently affects immigrants more (signaling need for targeted support).
